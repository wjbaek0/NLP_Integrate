...
def get_data(ratings_train,ratings_vaild,ratings_test):
	...
	train_data.loc[(train_data['label'] == "chemical"), 'label'] = 4  # 유해화학물질사고 => 6
	...
	val_data.loc[(val_data['label'] == "chemical"), 'label'] =  4  # 유해화학물질사고 => 6
	...
	test_data.loc[(test_data['label'] == "chemical"), 'label'] = 4 # 유해화학물질사고 => 6
...
if __name__=='__main__':
	...
	dataset_train, dataset_val , dataset_test = get_data('/home/xaiplanet/xai_workspace/nlp_integrate/Classification/output_data/ratings_train.csv',
                                                      '/home/xaiplanet/xai_workspace/nlp_integrate/Classification/output_data/ratings_val.csv',
                                                      '/home/xaiplanet/xai_workspace/nlp_integrate/Classification/output_data/ratings_test.csv') # csv 데이터 불러오기 
	...
	if Task == 1 :
		make_dirs()
		# BERT 모델 불러오기
		model = BERTClassifier(model, num_classes=8, dr_rate=drop_rate).to(device)
		...
		pre_model.load_state_dict(torch.load('./model/kobert_model_49.pth'), strict=False) # Weight 경로확인
		# pre_model.load_state_dict(torch.load('./model/221215_1958/kobert_model_49.pth'), strict=False) # Weight 경로확인
		...
		checkpoint = torch.load('./model/221215_1958/kobert_model_DenseNet_ckpt.pt') # ckpt 경로 확인
