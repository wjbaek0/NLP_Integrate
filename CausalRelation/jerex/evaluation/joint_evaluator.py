...
class JointEvaluator(Evaluator):
    ...
    def compute_metrics(self, ground_truth, predictions):
        ...
        # mention_eval = scoring.score(gt_mentions, pred_mentions, print_results=False) #MEMO print false
        ...
        # coref_eval = scoring.score(gt_clusters, pred_clusters, print_results=False) #MEMO print false
        ...
        # entity_eval = scoring.score(gt_entities, pred_entities, type_idx=1, print_results=False) #MEMO print false
        ...
        # rel_eval = scoring.score(gt_relations, pred_relations, type_idx=2, print_results=False) #MEMO print false
        ...
        rel_nec_eval = scoring.score(gt_relations_et, pred_relations_et, type_idx=2,
                                     print_results=True)
        # rel_nec_eval = scoring.score(gt_relations_et, pred_relations_et, type_idx=2,
        #                              print_results=False) #MEMO print false
    ...
    def store_predictions(self, predictions, documents, path):
        ...
        for doc_predictions, doc in zip(predictions, documents):
            ...
            print(doc_converted_predictions['entities'])
        ...
        #MEMO prediction 저장 한국어되게끔
        # with open(path, 'w') as predictions_file:
            # json.dump(converted_predictions, predictions_file, ensure_ascii=False)

        #MEMO 한번더씀
        # causal_path = "../../../../infer_result/"
        # causal_path = "./infer_result/"
        causal_path = "/home/xaiplanet/xai_workspace/nlp_integrate/CausalRelation/data/runs/infer_result/"
        os.makedirs(causal_path, exist_ok=True)

        #MEMO prediction 저장 한국어되게끔
        with open(f"{causal_path}predictions.json", 'w') as predictions_file:
            json.dump(converted_predictions, predictions_file, ensure_ascii=False)
