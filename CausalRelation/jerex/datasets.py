...
class DocREDDataset(TorchDataset):
    ...
    def _parse_document(self, doc):
        ...
        # parse tokens
        sentences, doc_encoding = self._parse_sentences(jsents) #MEMO inference할떄 이것만 있으면 되는거 아냐??

        # parse entity mentions
        entities = self._parse_entities(jentities, sentences) #MEMO entity 할때 vertexSet와 합쳐진 sentence list 들어감

        # parse relations #MEMO parse relations
    ...
    def _parse_sentences(self, jsentences): #MEMO datasets 쪼개저 있는 sentence를 넣는다. 리스트로 되어 있는데 문단으로 쪼개져있음
        sentences = []
        ...
        for sidx, jtokens in enumerate(jsentences):
            sentence_tokens = []
            #MEMO 여기서 단어 한개 씩 토큰화!!
            for tok_sent_idx, token_phrase in enumerate(jtokens):
                token_encoding = self._tokenizer.encode(token_phrase, add_special_tokens=False) #MEMO tokenizer.encode
                if not token_encoding:
                    token_encoding = [self._tokenizer.convert_tokens_to_ids('[UNK]')]
                span_start, span_end = (len(doc_encoding), len(doc_encoding) + len(token_encoding)) # MEMO span_start, span_end 번호 만들어줌?
    ...
    def _parse_entities(self, jentities, sentences) -> List[Entity]:
        ...
        for entity_idx, jentity in enumerate(jentities):
            ...
            for jentityMention in jentity:
                ...
                start, end = jentityMention['pos'][0], jentityMention['pos'][1] #MEMO entity start end

                # create entity mention
                sentence = sentences[jentityMention['sent_id']] #MEMO sent_id로 sentence 찾아감
                tokens = sentence.tokens[start:end]._tokens     #MEMO start와 end로 entity 찾아냄
