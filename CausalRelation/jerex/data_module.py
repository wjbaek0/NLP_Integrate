...
class DocREDDataModule(pl.LightningDataModule):
    ...
    def __init__(self, tokenizer: BertTokenizer, task_type: str, types_path: str = None,
                 train_path: str = None, valid_path: str = None, test_path: str = None, pred_path : str = None, ##
                 entity_types: dict = None, relation_types: dict = None,
                 train_batch_size: int = 1, valid_batch_size: int = 1, test_batch_size: int = 1, pred_batch_size : int =1, ##추가 
        ...
        if types_path is not None:
            ...
            for i, (key, v) in enumerate(types['entities'].items()):
                ...
                print("========DATA_MODULE.py's entity_type result=========")
                print(entity_type)
        ...
        self._pred_batch_size = pred_batch_size ## 추가 
    ...
    def setup(self, stage=None):
        ...
        ##  pred 추가 
        if (stage == 'pred' or stage is None) and (self._pred_path is not None or
                                                   (self._final_valid_evaluate is True and
                                                    self._valid_path is not None)):
            if self._pred_path is not None:
                self._pred_dataset = DocREDDataset(dataset_path=self._pred_path,
                                                   entity_types=self._entity_types,
                                                   relation_types=self._relation_types,
                                                   max_span_size=self._max_span_size,
                                                   tokenizer=self._tokenizer)
            else:
                self._pred_dataset = self._valid_dataset

            self._pred_dataset.switch_task(self._task_type)
            self._pred_dataset.switch_mode(DocREDDataset.INFERENCE_MODE)
    ...
    def pred_dataloader(self):
        return DataLoader(self._pred_dataset, batch_size=self._pred_batch_size, shuffle=False, drop_last=False,
                          num_workers=self._sampling_processes,
                          collate_fn=collate_fn_padding)
