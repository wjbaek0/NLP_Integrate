...
def predict(model,test_dataloader):
    ...
    for index,(i,j) in enumerate(zip(y_true_sort,y_pred)):
        print(f"index = {index+2}, y_true = {i}, y_pred = {j}\n")
        if i == j :
...
if __name__=='__main__':
    ...
    dataset_train, dataset_val , dataset_test = get_data('output_data/ratings_train.csv','output_data/ratings_val.csv', 'output_data/ratings_test.csv') # csv 데이터 불러오기 
    ...
    train_dataloader = torch.utils.data.DataLoader(data_train, batch_size=batch_size, num_workers=0, shuffle=True) # cpu = 0
    valid_dataloader = torch.utils.data.DataLoader(data_val, batch_size=batch_size, num_workers=0, shuffle=True) # cpu = 0
    test_dataloader = torch.utils.data.DataLoader(data_test, batch_size=batch_size, num_workers=0, shuffle=True) # cpu = 0
    ...
    if Task == 1 :
        ...
        pre_model.load_state_dict(torch.load('./model/221220_1353/kobert_model_20.pth'), strict=False) # Weight 경로확인
